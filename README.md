...
| **Status** | **33 active members across 51 slots**, gated by `scripts/apply.sh` on `v7.2`. Nine byte-verified members carry `rk3588-media-island v2026.9.4`. This lane bump is software-only: board qualification has not run, and the downstream image pin PR must stay open through that gate. |
...
byte-for-byte from its `v2026.9.4` release asset. The `Island(...)` variant and
independent verifier keep that provenance distinct from upstream commits,
first-party raw diffs, and unmerged lore postings.

The release replaces RGA's shared page-table ring with job-owned tables, requires
execution-device DMA mappings on RGA2, and bounds queued admission and timeout
cancellation. Failed reset is fail-closed: memory and power are retained on the
faulted core until reboot; unload refuses rather than waiting indefinitely.
See the [island release](https://github.com/CERALIVE/rk3588-media-island/releases/tag/v2026.9.4)
for software evidence, the unchanged MPP-checker limitation, and the unrun board
gate. This repository proves asset provenance and patch application, not hardware
behavior. A bounded re-reset is not a qualified recovery path.
