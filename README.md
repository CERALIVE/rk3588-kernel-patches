...
documents building `tsukumijima/mpp-rockchip`. **Untested by us** — we gate patch
application only.
...
repackages the driver as a DKMS module. We deliberately do **not** use it:
...
- **Hardware-untested, and largely machine-generated.** Its history
  ...
  automated extraction. We found no evidence of validation on an RK3588 board.

Forking `rcawston` directly keeps us on the maintained line, with the encoder
patch at v3 and the audio patch present.
...
The short version: the new encoder sources carry
`SPDX-License-Identifier: (GPL-2.0+ OR MIT)` and `MODULE_LICENSE("Dual MIT/GPL")`,
and we verified that the Rockchip BSP originals they were ported from carry the
**same** SPDX tags — so the dual-licence claim is inherited from the copyright
holder, not invented downstream. Distributing this as GPL-2.0 kernel patches is
the ordinary, safe case, and that is what we do.

The honest caveat: the upstream patch repository has **no `LICENSE` file at all**,
so there is no collection-level grant, and we did not perform a line-by-line
derivation audit of the ~4,200 ported lines. Anyone wanting to rely on the **MIT**
branch of that disjunction needs their own audit and actual legal review. No legal
sign-off is claimed here — `docs/PROVENANCE.md` is a factual ledger, including its
open questions.
