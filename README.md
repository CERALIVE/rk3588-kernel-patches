...
| `0030` | Orange Pi 5 Plus Type-C dual-role-power PDOs | `ceralive/` | **Pinned-source gap confirmed; patch application proven; live-tested on a REAL Orange Pi 5 Plus against the patched v7.2 kernel with a `PARTIAL` result — the PDO capability fix itself is proven working, the camera-attach regression status is UNRESOLVED.** The Orange Pi's own `rk3588-orangepi-5-plus.dts` declares a 5 V / 1.4 A fixed source PDO and a 5 V / 10 mA fixed sink PDO with `PDO_FIXED_USB_COMM` only. Live pre-patch readback at the identical `fusb302`/`fcs,fusb302` controller (`6-0022`; Rock is `4-0022`) reports read-only `dual_role_power=0` for both capability records. `0030` ORs `PDO_FIXED_DUAL_ROLE` into those two declarations without copying the Rock's different current values. This is Orange-Pi-only, edge-v7.2-only capability parity with `0028`; it does not touch role preferences, roles, status, any Rock DTS, or vendor 6.1. The adaptive policy does not attempt PR_SWAP on either board, so no PR_SWAP behaviour or reliability is claimed. **Hardware result, 2026-08-27, patched v7.2 on a REAL Orange Pi 5 Plus (`7.2.0-ceralive-rk3588`), port left `dual`.** Build and RAUC deploy both exited `0` and the new slot booted clean and healthy. The patch's own effect is proven directly and independently of any camera behaviour: `dual_role_power` now reads `1` on **both** `source-capabilities` and `sink-capabilities` under `.../i2c-6/6-0022/usb_power_delivery/pd0/`, where the pre-deployment kernel read `0` on both. The adaptive Type-C policy survived the deploy intact — `port_type` still `[dual] source sink`, `ceralive-typec-policy.service` still enabled, the legacy force-source unit still absent. **The camera no-regression check did NOT complete, and no regression claim is made in either direction.** The DJI Osmo Pocket 3 was attached and enumerated before the deploy (USB `7-1`, `2ca3:0023`), and was absent after the patched boot and again on a separate read-only recheck: `partner: ABSENT`, no UVC video nodes, FUSB302 IRQ count `0`, and a TCPM trace showing only startup through `SRC_UNATTACHED`/`Start toggling` with no CC attach transition. No physical replug cycle was available in the observation window, so this evidence cannot distinguish a camera-side power/off state from a boot-introduced Type-C attach regression, and it does not try to. **Regression status is unknown, pending a physical retest** — deliberately recorded that way rather than as a pass. **Exact next action for whoever picks this up:** with this patched slot still booted, perform a real full detach of at least 10 seconds and a reattach while observing Type-C/USB state. If the camera re-enumerates as `2ca3:0023` with `port_type` still `[dual]`, record the cycles and close the check; if it stays absent, that is a reproducible regression. Write no role sysfs control in either case. Measured on a manually test-built kernel with a temporary, uncommitted `patches_commit` test pin; nothing here makes `0030` a shipped default, and the vendor 6.1 track gains none of it. |
