...
| **Status** | **All 22 active members across 37 slots `git am` clean and every post-apply assertion passes on `v7.2`.** The published `rk3588-media-island v2026.9.1` release supplies the MPP encoder/decoder/JPEG driver source and in-tree DT integration through seven byte-verified members; it adds the island's job instrumentation and the three fault fixes its reliability drill found, and ships under the Phase-3 base verdict KEEP (reversal gate G1 NOT-FIRED). The image pipeline uses this series for the shipped, permanent mainline 7.2 production kernel. Board results quoted here from the previous `v7.1.7` base are historical. |
...
byte-for-byte from its `v2026.9.1` release asset. The `Island(...)` variant and
