...
| **Status** | **33 active members across 51 slots**, gated by `scripts/apply.sh` on `v7.2`. Nine byte-verified members carry `rk3588-media-island v2026.9.5`, which closes an RGA job-lifecycle race that could corrupt memory under sustained composition load. Rock 5B+ `edge-test` qualification passed on 2026-09-19 (8 clean + 8 engine-restart cycles + a 300 s soak with encoder/decoder/RGA loaded, no kernel fault report) and the board was restored to production. |
...
| `0011` | dw-hdmi-qp N/CTS helper | `backports/` | **Merged upstream — release projected, still carried** (Simon Wright, `PATCHv3`). Drops dw-hdmi-qp's private audio N/CTS table, which disagrees with the shared helper at several TMDS rates, for `drm_hdmi_acr_get_n_cts()`. Applied by Heiko Stuebner 2026-09-03 as `a9f09b5ea0c3` and reached through **linux-next**; it is in neither `v7.2` (our pin) nor Linus's tree, so **no release carries it yet** and `7.4-rc1` is a projection to re-check, never a fact. Stays in `backports/` because `v7.2` lacks the helper and the patch applies clean. Retire when the pinned base absorbs it — a content check, never a version comparison. Full evidence in [`docs/UPSTREAM-STATUS.md`](docs/UPSTREAM-STATUS.md). |
...
byte-for-byte from its `v2026.9.5` release asset. The `Island(...)` variant and
independent verifier keep that provenance distinct from upstream commits,
first-party raw diffs, and unmerged lore postings.

The release closes a race in RGA job lifecycle management: an admitted job's
allocation could be released by completion while the submitting path was still
reading it, which surfaces as intermittent memory corruption under sustained
composition load. The queue reference is now taken before publication and
released after the last use, and pre-publication failures route through the same
destructor. The earlier job-owned-table and execution-DMA repairs are unchanged,
and failed reset remains fail-closed: memory and power are retained on the
faulted core until reboot, and unload refuses rather than waiting indefinitely.
See the [island release](https://github.com/CERALIVE/rk3588-media-island/releases/tag/v2026.9.5)
for software evidence and the unchanged MPP-checker limitation. Rock 5B+
`edge-test` qualification passed on 2026-09-19 with encoder, decoder and RGA all
loaded — 8 clean composition cycles, 8 engine-restart cycles and a 300 s soak
with no kernel fault report. This repository proves asset provenance and patch
application, not hardware behavior. A bounded re-reset is not a qualified
recovery path.
