...
SERIES: tuple[Patch, ...] = (
    ...
    Patch(
        ...
        ordinal=22,
        subject=(
            "media: rockchip: rkvenc: refuse register requests the class map "
            "does not cover"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
        ...
        rationale=(
            ...
            "included.",
            "",
            "BEHAVIOUR. req_fully_covered() compares the summed clamped parts",
            "with the request's own size; reg_msg[]'s classes are disjoint, so",
            "equality is an exact containment test and a request naming an",
            "unowned byte is -EINVAL instead of being quietly truncated. Both",
            ...
            "or 0016's shape and window checks -- a request that lies wholly",
            "inside the classes it names behaves exactly as before, which is",
            "what librockchip-mpp sends. Does not make the class map's",
            "inclusive/half-open mismatch between req_over_class() and",
            "rkvenc_result() consistent; that is pre-existing and separate.",
            "Does NOT fix the valid-after-failures case, which fails for an",
            "unrelated reason recorded in docs/UPSTREAM-STATUS.md.",
            "",
            ...
            "",
            "EVIDENCE POINTER. Root-caused from a REAL Rock 5B+",
            "rkvenc-invalid-ioctl --all-malformed transcript. No test",
            "expectation changes: tests/expected-errno.tsv already demanded",
            "these values and the driver did not deliver them. Build-clean;",
            "hardware re-verification is pending. See",
            "docs/BOARD-QUALIFICATION.md.",
        ),
    ),
)
