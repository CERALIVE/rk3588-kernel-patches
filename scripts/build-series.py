...
SERIES: tuple[Patch, ...] = (
    ...
    Patch(
        ...
        ordinal=21,
        subject=(
            "media: rockchip: rkvenc: release only what the task actually "
            "acquired"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
    ...
    Patch(
        ...
        ordinal=22,
        subject=(
            "media: rockchip: rkvenc: refuse register requests split across "
            "map holes"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
        ...
        rationale=(
            ...
            "included.",
            "",
            "The first version of this patch then broke every hardware encode,",
            "and this is the amended one. It required the summed clamped parts",
            "to EQUAL the request's size, asserting that \"a request that lies",
            "wholly inside the classes it names behaves exactly as before,",
            "which is what librockchip-mpp sends\". A cold-boot control encode",
            "on a real Rock 5B+ -- nothing armed, no fault injected -- proved",
            "that premise FALSE. MPP's class BASE write is offset 0 size 96;",
            "reg_msg[] owns [0x0000,0x005c), which is 92 bytes, so the",
            "request's last dword lands in the 137-dword hole between BASE and",
            "PIC. Every production task was refused (\"write request",
            "00000000+96 names 4 bytes no register class owns\", then \"alloc",
            "task failed: -22\") for 0 bytes out, against 1,854,524 bytes from",
            "the same board one RAUC slot earlier on the 19-patch kernel.",
            "reg_msg[] is a SPARSE map -- no two of its nine classes abut, the",
            "holes run from six dwords (SQI to SCL) to 770 (PIC to RC) -- so",
            "the clamp has silently dropped edge dwords since 0001 imported",
            "it. An equality test cannot tell that pre-existing, harmless drop",
            "apart from a real malformation.",
            "",
            "BEHAVIOUR. req_coverage_check() asks WHERE the dropped bytes went",
            "instead of how many there were. The clamped parts are disjoint",
            "subranges of the request, so they sum to the span they cover only",
            "when that span has no hole inside it; requiring sum == span is",
            "exactly \"the split consumed ONE contiguous run\". A request that",
            "merely spills off a class's edge into the neighbouring hole --",
            "MPP's 96-byte BASE write -- is one run, so it is accepted and",
            "clamped exactly as it was before 0022. A request stitched from",
            "several runs with the map's holes BETWEEN them, which is what",
            "class-overrun sends, is still -EINVAL. Contiguity alone cannot",
            "catch a request running past the LAST class, because no following",
            "class is there to notice, so the request is also bounded by the",
            "map's own extent -- computed from reg_msg[], not hardcoded, so a",
            "map with different holes or none needs no change. Both request",
            "counts are bounded by their own arrays. INIT_TRANS_TABLE and",
            "SET_REG_ADDR_OFFSET both require a whole number of elements, and",
            "the latter now bounds bytes against the room left rather than",
            ...
            "this check refuses a misleading SHAPE. Does not make the class",
            ...
            "counterpart exists.",
            "",
            "EVIDENCE POINTER. Both halves came off a REAL Rock 5B+: the",
            "rkvenc-invalid-ioctl --all-malformed transcript that motivated",
            "the patch, and the cold-boot control encode plus one-slot-apart",
            "A/B that caught it breaking production. class-overrun,",
            "trans-table-odd-size and bad-user-pointer are confirmed FIXED on",
            "hardware, and this amendment keeps all three rejected: replayed",
            "on the host against the real reg_msg[] table for every case in",
            "tests/expected-errno.tsv plus MPP's own request shape. No test",
            "expectation changes. THIS AMENDMENT IS HOST-VERIFIED ONLY --",
            "build-clean at W=1, and a cold-boot no-fault control encode is",
            "the required next hardware step. That control is the check this",
            "series should run for EVERY rkvenc UAPI change, not just the",
            "fault cases; it is what caught this one. See",
            "docs/UPSTREAM-STATUS.md and docs/BOARD-QUALIFICATION.md.",
        ),
    ),
    Patch(
        filename="0023-rkvenc-worker-task-lifetime.patch",
        ordinal=23,
        subject=(
            "media: rockchip: rkvenc: stop reading a task the worker has "
            "already released"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
        date="Tue, 11 Aug 2026 15:30:00 -0500",
        origin=CERALIVE,
        rationale=(
            "MOTIVATION. A KASAN+lockdep Rock 5B+ running",
            "tests/rkvenc-fault-qa.sh --case fail-clock-enable reported 'BUG:",
            ...
            "rkvenc_task_finish() ends in kref_put(&task->ref,",
            "rkvenc_free_task_callback), which kfree()s the whole struct",
            "rkvenc_task. A task carries exactly two references, the",
            "allocator's and the one rkvenc_dev_ioctl() takes for the waiter,",
            "so whichever of the worker and the waiter puts last is the one",
            "that frees -- and the worker keeps using the pointer either way.",
            "Its IRQ arm calls rkvenc_task_finish() and the timeout arm",
            "immediately below re-reads mpp_task->state; state sits at offset",
            "0x30 of the allocation behind three list_heads, test_bit() is an",
            "8-byte instrumented read, and sizeof(struct rkvenc_task) is 11408",
            "bytes, so the object comes from the page allocator page-aligned --",
            "which is the address, the width and the page-not-slab report",
            "KASAN printed, to the byte. Its hw_run-failure arm calls",
            "rkvenc_task_finish() and then wake_up(&pending_task->wait), the",
            "same defect one field along.",
            "",
            "BEHAVIOUR. The worker stops touching a task it has handed to",
            "rkvenc_task_finish(). The IRQ arm nulls its local afterwards, so",
            "the timeout arm cannot read the freed state word. No reachable",
            "work is skipped: rkvenc_hw_irq() and rkvenc_task_timeout_work()",
            "both claim a task with test_and_set_bit(TASK_STATE_HANDLE), and",
            "the timeout side does so with the encoder's IRQ disabled, so",
            "TASK_STATE_IRQ and TASK_STATE_TIMEOUT are never both set on one",
            "task -- and had that gate ever broken, the old code would have",
            "called rkvenc_task_finish() twice on it and put one reference",
            "twice. The hw_run-failure arm loses its trailing wake_up():",
            "rkvenc_task_finish() sets TASK_STATE_DONE and wakes task->wait on",
            "every path, including 0021's teardown-skipping one, so the waiter",
            "is already awake and the second wake only read a freed wait queue.",
            "",
            "NON-GOALS. Does not change the reference-counting model: two",
            "references, two owners and three drop sites, exactly as 0001 and",
            "0014 left them, and no kref_get() is added to hold the window open",
            "instead of closing it. Does not touch 0021 -- its",
            "TASK_STATE_HW_HELD bookkeeping is correct and hardware-confirmed,",
            "and reverting it would trade this bug for the rwsem and",
            "runtime-PM imbalance it fixed. Does not touch rkvenc_task_finish()",
            "itself, 0014's quiesce/drain, or 0020's per-core requiesce. No new",
            "lock, no new wait, no fault-injection knob.",
            "",
            "PROVENANCE. First-party CeraLive fix. Both dereferences are 0001's",
            "as imported and no patch between them changes either: 0013 inserts",
            "its delay BEFORE the IRQ arm's finish, 0019 changes only the queue",
            "lock types, 0021 adds one atomic_inc() before the failure arm's",
            "finish and jumps into the same unconditional kref_put() tail. What",
            "0021 changed is reachability -- before it this path double-released",
            "the reset rwsem and the PM references, the encode wedged, and the",
            "worker never ran on far enough to be caught here. No upstream",
            "counterpart exists.",
            "",
            "EVIDENCE POINTER. Root-caused from a REAL Rock 5B+ KASAN report,",
            "then pinned to the line on the applied v7.1.7 tree: pahole on a",
            "KASAN+PROVE_LOCKING arm64 build of this exact series puts",
            "rkvenc_mpp_task.state at offset 48 size 8 and struct rkvenc_task",
            "at 11408 bytes, and 0xffff00010da88030 is 0x30 into a 16 KiB-",
            "aligned allocation. wait is at offset 112, so the report names the",
            "timeout arm's test_bit() and not the failure arm's wake_up(); both",
            "are fixed. HOST-VERIFIED ONLY -- build-clean at W=1 and gate-clean,",
            "with NO hardware re-run. The fault-QA harness already asserts the",
            "correct behaviour, so no test expectation changes. See",
            "docs/UPSTREAM-STATUS.md and docs/BOARD-QUALIFICATION.md.",
        ),
    ),
    Patch(
        filename="0024-rkvenc-secondary-core-iommu-domain-lifetime.patch",
        ordinal=24,
        subject=(
            "media: rockchip: rkvenc: keep a secondary core off the queue "
            "while it has no IOMMU domain"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
        date="Tue, 11 Aug 2026 17:55:00 -0500",
        origin=CERALIVE,
        rationale=(
            "MOTIVATION. A KASAN+lockdep Rock 5B+ Oopsed inside",
            "rkvenc_iommu_attach(): 'KASAN: null-ptr-deref in range",
            "[0x20-0x27]', pc __iommu_attach_group+0x15c, x20 = x22 = 0,",
            "x0 = 0x20, reached from rkvenc_hw_run+0x22c and",
            "rkvenc_task_worker_default+0x828. struct iommu_domain lays out",
            "type at 0x00, cookie_type at 0x04, is_iommupt at 0x08, ops at",
            "0x10 and dirty_ops at 0x18, so owner is at 0x20 -- and",
            "__iommu_attach_group() reads exactly that, through",
            "domain_iommu_ops_compatible(), before it does anything else. The",
            "faulting load is domain->owner with domain == NULL.",
            "",
            "RK3588 wires two encoder cores behind one CCU. The secondary",
            "borrows the main core's IOMMU domain at probe. When the MAIN core",
            "unbinds, rkvenc_core_unwind() detaches every secondary and NULLs",
            "its domain -- correctly, the page tables are going away. When the",
            "main core comes BACK it re-probes into the !ccu->main_core arm of",
            "rkvenc_attach_ccu(), which only claims the main slot; the arm that",
            "shares a domain is the else, and only a probing SECONDARY reaches",
            "it. The secondary never re-probes, so its domain stays NULL for",
            "the rest of the boot -- while it is still in queue->cores[] with",
            "its idle bit set, because the unwind's queue step clears the slot",
            "of the core that is UNBINDING and that is not this one. The next",
            "task the worker sends to it Oopses. On the board this took three",
            "unbind/rebind cycles (QA commands 02/03/04) followed by any encode",
            "that reached the second core.",
            "",
            "BEHAVIOUR. A core is dispatchable if and only if it has a usable",
            "IOMMU domain. The unwind unpublishes a secondary at the moment it",
            "takes its domain away, and a core that becomes main re-shares its",
            "domain with the secondaries already on ccu->core_list and",
            "republishes each one that takes it. rkvenc_iommu_attach() also",
            "refuses a NULL domain or group outright rather than handing it to",
            "iommu_attach_group(): the pre-existing comparison cannot catch",
            "that case, because iommu_get_domain_for_dev() returns the core's",
            "DEFAULT domain, which is never NULL, so NULL != default and the",
            "call went ahead. rkvenc_hw_run() then unwinds through 0021's",
            "balanced err_unlock and the task fails as -ENODEV instead of",
            "taking the machine down. Publication and unpublication become one",
            "idempotent pair of helpers, keyed on the stage bit they already",
            "owned, so the four call sites cannot drift.",
            "",
            "NON-GOALS. Does not change cold boot: the first core to probe",
            "finds an empty ccu->core_list and the re-share is a no-op, so",
            "initial bring-up runs the same code it always did. Does not touch",
            "the reset group, the worker, task lifetime or 0021/0022/0023.",
            "Does NOT fix the 'possible recursive locking' warning on",
            "&rg->rw_sem that accompanies this Oops: that is a separate defect",
            "-- rkvenc_hw_run() returns holding down_read() and the single",
            "rkvenc-worker kthread necessarily nests it when it dispatches to",
            "the second core, because find_first_bit() only picks core 1 while",
            "core 0 is busy. It is reported once per boot because lockdep",
            "disables itself afterwards, it cannot deadlock today (nothing",
            "takes that rwsem for write), and it is tracked separately. No new",
            "lock is introduced: the helpers take queue->dev_lock under",
            "ccu->lock, the same order rkvenc_core_unwind() already uses, and",
            "dev_lock is taken nowhere else.",
            "",
            "PROVENANCE. First-party CeraLive fix. The NULL-domain window is",
            "0020's as written -- it made the service survive a single core's",
            "unbind and cleared the sharing stage bit, but nothing re-sets it",
            "-- and the unguarded attach is 0001's as imported. No upstream",
            "counterpart exists.",
            "",
            "EVIDENCE POINTER. Root-caused from a REAL Rock 5B+ Oops and",
            "confirmed against the applied v7.1.7 tree: the reported fault",
            "offset is struct iommu_domain's owner member, and the boot log",
            "shows three 'attach ccu as core 0 [main]' lines after the",
            "unbind cycles and not one 'attach ccu as core 1', which is the",
            "NULL domain being created and never repaired. Re-verified ON",
            "HARDWARE after this patch: the unbind/rebind-then-encode sequence",
            "that produced the Oops ran clean repeatedly with zero KASAN,",
            "zero Oops and zero recursive-locking reports, and both cores were",
            "confirmed dispatchable again afterwards. No test expectation",
            "changes. See docs/UPSTREAM-STATUS.md and",
            "docs/BOARD-QUALIFICATION.md.",
        ),
    ),
    Patch(
        filename="0025-rkvenc-service-node-teardown-lifetime.patch",
        ordinal=25,
        subject=(
            "media: rockchip: rkvenc: keep the service alive until its "
            "last descriptor closes"
        ),
        provenance=NULL_OID,
        author="CeraLive <dev@ceralive.tv>",
        date="Tue, 11 Aug 2026 22:45:00 -0500",
        origin=CERALIVE,
        rationale=(
