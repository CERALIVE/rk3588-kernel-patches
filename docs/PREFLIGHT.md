...
The `--head` form is the one that answers *"has Armbian moved `edge` since we
pinned?"*, and it is what CI runs on a schedule.
...
rolling stable branch. `preflight.sh` watches for an override appearing on our
...
| **Our pin** | **`v7.1.5`** = `155b42bec9cbb6b8cdc47dd9bd09503a81fbe493` |

### Why we pin a tag when Armbian uses a branch
