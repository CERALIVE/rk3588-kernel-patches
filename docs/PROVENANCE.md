...
This is a **factual ledger**, not legal advice and not a clearance. It records what
we looked at, what we found, and which questions we could not close. No lawyer has
reviewed it and no legal sign-off is claimed or implied. Where something is
uncertain it is written down as uncertain.

---

## 1. What we redistribute
...
| Our own work | `scripts/`, `docs/`, `rebase/`, `kernel-pin.env`, `.github/` | CeraLive packaging, tooling, and documentation |

We redistribute **no compiled kernel, no `.deb`, and no binary blob**. This
repository produces patch text only.
...
The headers say "Ported from Rockchip BSP …". We checked that claim against the
...
was therefore **inherited from the copyright holder's own header**, not invented
downstream by the porter. That is the ordinary, expected shape of a vendor-code
port and it is the answer to the obvious first question about a `MIT` claim on
kernel code.
...
port. We record them rather than tidy them, because **this repository does not
modify patch content** (see §6).

---

## 5. What we can say about redistribution

**GPL-2.0 distribution of this series is the safe, ordinary case.** Reasoning,
...
- We therefore treat the combined work (kernel + this series) as **GPL-2.0-only**,
  and this repository is distributed on that basis.

### 5.1 The MIT-claim caveat, stated plainly

The `MIT` half of the disjunction is a **real, unresolved ambiguity**, and we are
...
- **The SPDX tag is inherited, which is evidence but not proof.** We verified that
  Rockchip's own BSP files carry `(GPL-2.0+ OR MIT)` (§4). That is strong evidence
  the dual grant is genuine and originates with the copyright holder. It is *not*
  proof that every line in the ported files came from those specific dual-licensed
  BSP files — a port is a rewrite, and we did not perform a line-by-line
  derivation audit of ~4,200 lines of driver code against the BSP originals.
- **A GPL-2.0-only line anywhere in the port would silently collapse the
  ...
  header. We have not ruled this out.
...
**Practical consequence for CeraLive:** none, today. We use the GPL-2.0 branch, we
ship the series as kernel patches, and we make **no MIT claim** and take no
MIT-dependent action anywhere. If anyone ever wants to rely on the MIT branch —
for instance to link this code into something proprietary — that would need its
own derivation audit and actual legal review. This document is not that.

---

## 6. What this fork does and does not change

- We do **not** modify patch behaviour. `patches/` is a repackaging of
  `upstream/`; every added and removed line is byte-identical, mechanically
  enforced by `scripts/verify-payload-parity.py` in CI.
- We do **not** add, remove, or alter any SPDX identifier, copyright line, or
  `MODULE_LICENSE` value.
- We do **not** relicense anything. `LICENSE` in this repository describes the
  terms the imported material already carries; it grants nothing new.
- We do **not** claim this series is upstream-mergeable, or that it has been or
  will be offered to `rcawston` or to the Linux kernel. Upstream's own README
...
2. Whether Rockchip's BSP `(GPL-2.0+ OR MIT)` tags are themselves accurate for
   every line of *their* sources. Out of scope; we take the copyright holder's
