...
`0031`–`0039` (2026-09-03), `0040`–`0049` — the EDID streaming guard, AVI
...
**Rows added since, 2026-08-13:** `0027` — added already `VALIDATED` on Rock 5B+,
the first row in this file to carry a real 4K59.94p capture as its evidence.

**Full sweep, 2026-08-26 — the first at the `v7.2` base, and the first run
differentially against a saved baseline.** All twenty-two member rows were
re-resolved against a live source on that date rather than carried forward, and
each row's route is recorded rather than assumed: the `v7.2` tree itself for
content and path checks, `patchwork.kernel.org`'s API plus the canonical
`t.mbox.gz` thread archive for the four tracked postings, GitHub's
`compare/<sha>...v7.2` for containment, and the Collabora `mainline-status.md`
table for `0001`. **All twenty-two were reachable, and no row's status token
changed.** The base bump absorbed no counterpart of any member: the one commit
that landed in a member's file, `d1162a5adbb5`, is `0002`'s already-evaluated
counterpart and is treated in [§ `0002`](#0002--one-upstream-answer-already-in-the-base).

**Hardware validation is HISTORICAL from this sweep on, and every marker now says
so.** Every `VALIDATED` result in this file was measured on the `v7.1.7` base. The
base is `v7.2`, no board has run the re-anchored series, and a board result does
not transfer across a base bump by assertion — so each marker reads `VALIDATED at
v7.1.7 (historical); v7.2 board evidence pending`. Nothing was un-validated and
nothing was re-validated: the claim was re-scoped to the base it was actually taken
on. Re-running [`BOARD-QUALIFICATION.md`](BOARD-QUALIFICATION.md) at `v7.2` is what
clears "pending", and until a transcript exists **no `VALIDATED at v7.2` claim may
be written anywhere in this repository.**

**Capture mechanism, re-measured 2026-08-26 — the standing advice was wrong in both
directions, and the correction is one mechanism, not two.** Anubis keys its
interstitial on the **User-Agent**, so a browser-like UA is what draws the
challenge and `curl`'s own default UA is what gets served:

- The Collabora table's GitLab REST route —
  `…/api/v4/projects/<ns>%2Fnotes-for-rockchip-3588/repository/files/mainline-status.md?ref=main`
  — is **currently accessible without the Anubis gate (verified 2026-08-26)**: a
  plain `curl` and a `Mozilla/5.0` UA both returned HTTP 200 with the same
  37,103-byte JSON body, zero challenge pages. It resolved `main` to commit
  `607a267502837967ed4a9a58bd9cdaed842a9509`. The **browser fallback is retained**,
  not deleted: this file's older instruction to re-capture with a real browser
  remains the correct move *if* the gate returns, and the HTML table view still has
  no API of its own.
- `lore.kernel.org`'s `t.mbox.gz` route is the mirror image and the trap. Sent with
  a `Mozilla/5.0` UA it returns **HTTP 200 carrying an Anubis "Making sure you're
  not a bot!" challenge page** — a success status wrapping an HTML interstitial, so
  a fetch that only checks the status code reads as fine and yields no mbox. Sent
  with curl's default UA the same five URLs returned real gzip. **Do not "improve" a
  lore fetch by spoofing a browser UA**; that is precisely what breaks it.
...
[`docs/REBASE-v7.2.md`](REBASE-v7.2.md) (does it still apply, at the current base?) ·
...
**The vocabulary above is CLOSED.** Those five tokens are the whole set; a row's
**Upstream status** cell opens with exactly one of them. There is no `UNVERIFIED`
token and there must never be one — "we did not manage to check this" is not a
statement about upstream, and encoding it as a status would silently overwrite the
last real answer. A row whose source could not be reached keeps the status it
already had, and says so in its Notes instead (see the sweep semantics below).
...
**Sweep semantics — status and date move TOGETHER, or neither moves.** A sweep
re-resolves every member row against a live source, and each row lands in one of
exactly two states:

- **Reachable.** The source answered. The **Upstream status** is re-affirmed or
  corrected and **Last checked** moves to the sweep date. Both move together; a
  re-affirmed status is still a check, and it earns the new date.
- **Unreachable.** The source did not answer — a dead archive, a gated route, a
  timeout. Then **the status token and the date both stay byte-identical to what
  they were**, and the Notes gain one line: `re-check attempted <date> via
  <route>: unreachable`. Nothing else changes. The stale date is the honest signal
  that the row has not been re-verified, and overwriting it with the sweep date
  would destroy exactly the information the column exists to carry.

The failure this rule prevents is a sweep that walks the table, cannot reach three
sources, and moves all the dates anyway — after which nothing in the file can tell
a verified row from a skipped one.
...
| `0031` media-island maintained source | `island/` | `first-party-no-upstream` | A later island release replaces or removes this source component | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0032` media-island build hooks | `island/` | `first-party-no-upstream` | Mainline integration removes the out-of-tree hooks | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0033` Rockchip IOMMU provider exports | `island/` | `first-party-no-upstream` | Mainline exports equivalent provider APIs | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0034` IOMMU DMA IOVA accessor | `island/` | `first-party-no-upstream` | Mainline exposes an equivalent accessor | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0035` MPP encoder DT nodes | `island/` | `first-party-no-upstream` | Mainline owns the encoder nodes with a usable production ABI | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0036` MPP decoder DT ownership | `island/` | `first-party-no-upstream` | Decoder ownership returns to a production-capable mainline driver | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0037` MPP JPEG decoder DT node | `island/` | `first-party-no-upstream` | Mainline provides the required JPEG decode path | 2026-09-19 | From `rk3588-media-island v2026.9.5` |
| `0038` RGA3 DT ownership | `island/` | `first-party-no-upstream` | Mainline provides a production-capable multi-core RGA driver | 2026-09-19 | From `rk3588-media-island v2026.9.5`; owns RGA3 core0 and core1 |
| `0039` RGA2 DT ownership | `island/` | `first-party-no-upstream` | Mainline provides a production-capable RGA2 driver | 2026-09-19 | From `rk3588-media-island v2026.9.5`; owns RGA2 |
...
| `0051` hdmirx no-signal log level | `ceralive/` | `first-party-no-upstream` | Mainline stops logging an absent source at error level | 2026-09-12 | **Send upstream (linux-media).** `hdmirx_query_dv_timings()` logged `port has no link` and `signal is not locked` at `v4l2_err`; neither is an error — both are the resting state of an input with nothing plugged in, and each is already returned as `-ENOLINK`/`-ENOLCK`. Measured on an idle Orange Pi 5 Plus: **1476 of 1623** dmesg lines were the first message and 138 more the second — 99.5% of a 13-hour boot's entire kernel log from one polling path. Both sites become `v4l2_dbg(1, debug, …)` behind the driver's existing module parameter, which the file already uses in 62 places; return values and control flow are untouched. Matters beyond tidiness because `CONFIG_PSTORE_CONSOLE` rings a small reserved DRAM buffer: a repeated error-level message is precisely what evicts the pre-fault console the feature exists to preserve. |
<!-- current-series: end -->
...
**`0022` has been found broken by hardware TWICE, and amended in place both times.**
v1 fixed the three drill cases it targeted — confirmed on a board — and
simultaneously refused **every** production encode, because the containment test it
introduced was exact and `librockchip-mpp`'s real register write is not exactly
contained. A cold-boot control encode with nothing armed caught it; an A/B one RAUC
slot apart confirmed it. v2 replaced containment with "one contiguous run", which
freed H.264 and left **H.265 refused on every kernel that has ever carried `0022`** —
MPP's HEVC register programme is a single write spanning `SQI` and `SCL` with the
genuine 24-byte map hole between them, and one run it is not. v3 is the fix, and
unlike its two predecessors it was **proven on a board before it landed**: see
[§ `0022`](#0022--what-it-fixes-what-it-does-not-and-the-one-case-still-red).
The lesson generalises beyond this patch, and v2 is why it needed restating: a
**cold-boot, no-fault control encode** belongs in every rkvenc UAPI change's
acceptance — and it has to cover **every codec the board can be asked for**, because
v2 passed an H.264-only one and shipped a total H.265 outage behind it.
...
`0027` is the first first-party patch in this repository that was root-caused,
written, built, deployed and **proven on a board inside one session** — and it is
the first HDMI-RX patch here whose evidence is a working 4K60 capture rather than a
silenced report. It repairs the imported `snps_hdmirx` ratio logic that `0002`
inherited: the driver could not distinguish an SCDC bank the source never wrote from
an explicit 1/10 declaration, so a 4K59.94p link that a Rock 5B+ had failed to lock
~500 consecutive times was structurally unlockable rather than merely unlucky. It
also **answers B4 of [`EVAL-0002-EDID.md`](EVAL-0002-EDID.md)** — the `i == 300` PHY
re-init does not recover this failure and cannot, because that re-init ends by
re-deriving the ratio from the same empty register. Its retirement is governed by
mainline's ratio logic, not by `0002`'s lock loop.
...
carried twice. See its row in [`retired/REGISTRY.md`](../retired/REGISTRY.md).

The following three were carried as series members and are not any more. They are **not**
...
|---|---|---|
| Retired slot `0007` — IOMMU DTE fetch time limit | The RK3588 IOMMU `MMU_AUTO_GATING` workaround, now present in mainline commit `8d4346ecd495` in `v7.2` | Archived at `retired/0007-iommu-rockchip-disable-fetch-dte-time-limit.patch`; registry row in [`retired/REGISTRY.md`](../retired/REGISTRY.md) |
...
[`REBASE-v7.2.md` § Stable overlap](REBASE-v7.2.md#stable-overlap--d1162a5adbb5)
and [`EVAL-0002-EDID.md`](EVAL-0002-EDID.md); the `v7.1.7`-base version of the same
argument is [`REBASE-v7.1.7.md` § Stable overlap](REBASE-v7.1.7.md#stable-overlap--7dd27810eea0-and-why-it-is-not-a-conflict).

**Re-verified at the `v7.2` base, 2026-08-26 — the verdict survives the bump, and
one word of it changes.** The counterpart is no longer reaching us through a stable
backport: `d1162a5adbb5` is contained in `v7.2` directly (`compare` → `ahead`,
`behind_by: 0`, merge base `d1162a5adbb5` itself), and the base's
`snps_hdmirx.c:511` reads `msleep(100 + 50)`. `0002`'s own three symbols are still
absent at `v7.2`, so nothing of it was absorbed. **`EVAL-0002-EDID.md` needs no
change**, and its hardware-gated questions B1–B7 stay exactly where they were:
`0027` answered **B4** in the negative, and the remaining six still need a board and
an HDMI source.
...
**The v2 amendment.** `req_coverage_check()` replaced "how many bytes were dropped"
with "**where** they went", and split the population in two:
...
**Read this part second: v2 shipped a total H.265 outage, and v3 is the fix.** The
two populations above are not exhaustive, and the missing third one is legitimate
traffic. `librockchip-mpp`'s HEVC register programme issues **one** write of 3228
bytes at `0x2000`, which is `SQI` (`[0x2000,0x21e8)`, 488 bytes) plus `SCL`
(`[0x2200,0x2c9c)`, 2716) plus the genuine 24-byte `SQI`→`SCL` map hole between
them. It starts on `SQI`'s first byte, ends on `SCL`'s last, overruns nothing and
drops nothing at either edge — and 488 + 2716 = 3204 ≠ 3228, so "one contiguous run"
refused it:

```
rkvenc_extract_task_msg:472: write request 00002000+3228 covers 3204 bytes, not one run
rkvenc_dev_ioctl:1376: alloc task failed: -22
```

which `librockchip-mpp` surfaces as `mpp_service_cmd_send ioctl MPP_IOC_CFG_V1
failed ret -1 errno 22`. **Every H.265 encode on every kernel carrying `0022` has
failed this way**, at every geometry, since `7.1.7-ceralive-rk3588`. H.264 issues no
multi-class write, so it never noticed — and the H.264-only control encode v2 shipped
behind is exactly why nobody else did either.

**The v3 discriminator is clipping, not contiguity.** A broken span is now allowed
when `req_spans_whole_classes()` says every class the span touches lies **entirely**
inside it, so the only bytes dropped in the middle are addresses no register answers
to. A span that merely CLIPS a class is still refused — and clipping is precisely
what `expected-errno.tsv` says `class-overrun` *is*: "starts inside a class and
claims a size that runs past its end". `class-overrun`'s read leg is
`offset 0x0004, size 0x4000`; it starts one dword inside `BASE` and stops one dword
inside `ST`, clipping both, so it stays `-EINVAL`. The one-run test is kept ahead of
it, so the common single-class case never walks `reg_msg[]` at all.

**READ and WRITE keep the same rule, deliberately**, even though only a write was
observed failing. Both sides consume the CLAMPED per-class parts and neither re-reads
the caller's raw span: `rkvenc_extract_task_msg()` copies each part into the class
buffer that owns it, and `rkvenc_result()` walks `task->r_reqs[]` — the parts, not the
request — copying each one back out of its own class buffer, both bounded by `0016`'s
`rkvenc_class_reg_window()`. A multi-class read is therefore no more exposed than a
multi-class write. Giving the read side a stricter rule was considered and rejected:
it would be an asymmetry with nothing behind it, and the next MPP request shape that
happens to be a multi-class *read* would reproduce this outage exactly.

What v3 deliberately does **not** do: it does not tighten what the clamp has always
tolerated. A request may still spill into an adjacent hole by as much as that hole
holds — at either end, multi-class or not — and those bytes are still dropped
silently. That is the pre-`0022` contract, memory safety on this path is `0016`'s
window check, and this check's job is to refuse a misleading *shape*. It also does
not object to a whole-class span being large: a request naming `BASE` through `ST`
end to end is accepted, because it is honest about every byte it covers, and size is
not the property under test. Widening `reg_msg[]`'s `BASE` end to the 24 dwords MPP
actually writes remains the other half of the story and is still **not** done here.

**Status: v3 is HARDWARE-VERIFIED on a Rock 5B+ (2026-08-14), the first version of
this patch that was.** It was built as a module against the running
`7.1.7-ceralive-rk3588` — same `.config` read back from the board's own
`/proc/config.gz`, same `vermagic`, `CONFIG_MODVERSIONS=n` — and hot-swapped with
`rmmod`/`insmod`, against a control module built from the shipped series in the same
environment. `videotestsrc`-fed `mpph265enc`, with no camera attached:

| leg | module | result |
|---|---|---|
| H.264 1080p60 | shipped | rc=0, **3,874,116 B** |
| H.264 1080p60 | control (rebuilt shipped series) | rc=0, **3,874,116 B** — byte-identical, so the build environment is faithful |
| H.265 1080p60 | control | rc=124 (timeout), **0 B**, `covers 3204 bytes, not one run` |
| H.265 1080p60 | **v3** | rc=0, **3,866,032 B**, `ffprobe` `hevc`/Main/1920x1080/yuv420p, **120/120 frames**, full decode to null clean |
| H.265 720p30 | **v3** | rc=0, 849,384 B |
| H.265 4K59.94 | **v3** | rc=0, 8,738,240 B |
| H.264 4K59.94 | **v3** | rc=0, 8,872,096 B |
| H.264 1080p60 | **v3** | rc=0, **3,874,116 B** — byte-identical to shipped, so no regression |

and not one `rkvenc` line in `dmesg` across the whole v3 sweep. **The security
property was re-run on the same two modules and is unchanged:**
`rkvenc-invalid-ioctl --all-malformed` returns the *identical* verdict on control and
on v3 — `class-overrun`, `offset-size-wrap`, `undersized-word`, `unaligned-offset`,
`invalid-metadata`, `trans-table-odd-size` → `EINVAL`, `bad-user-pointer` → `EFAULT`,
and `valid-after-failures` still red for the reason below that is neither patch's.
The logic was additionally replayed on the host against the real `reg_msg[]` table
for 18 request shapes, including four adversarial near-misses of the H.265 shape
(starting one dword late, ending one dword early, reaching back into `PAR`, reaching
forward into `OSD`) — all four correctly refused.

The fix is **not persisted on that board**: `/lib/modules` still holds the shipped
module and the board was restored to it, so a rebuilt kernel through
`image-building-pipeline` is what carries this to a device.

**The generalisable lesson, now twice earned.** `0022` v1 passed every test written
for it and broke production anyway, because the tests were all *fault* cases and the
thing it broke was the happy path. v2 then passed the control encode that caught v1
and broke H.265 anyway, because that control ran **one codec**. A cold-boot,
no-fault control encode is a mandatory leg of every rkvenc UAPI change in this
series — and it has to exercise every codec the hardware can be asked for. A green
fault drill says nothing about whether the driver still encodes, and a green H.264
encode says nothing about H.265.
...
> **HISTORICAL as of the `v7.2` base — both stop conditions below have dissolved,
> and there is nothing left to import.** Re-checked 2026-08-26. Everything this
> section vetoed is now *in the base*, so the veto records why we did not backport
> it to `v7.1.7`, not a live decision.
>
> - **Stop condition 1 (prerequisite chain four deep) is moot.** All four
>   prerequisites resolve at `v7.2`: `I2S0_8CH_MCLKOUT_TO_IO`,
>   `RK3588_SYSGRF_SOC_CON6` and `rockchip_clk_add_grf()` are all present. There is
>   no chain to backport when the whole chain has landed.
> - **Stop condition 2 (the regression's fix had not landed) is answered — it
>   landed, and it is in the base.** `946352b2f88fd` *"clk: rockchip: rk3588: don't
>   disable unused I2S MCLK output gates"* (2026-06-28) is contained in `v7.2`
>   (`compare` → `ahead`, `behind_by: 0`), and the content confirms it rather than
>   the commit message alone: `clk-rk3588.c` at `v7.2` carries `CLK_IGNORE_UNUSED`
>   on **all four** `_TO_IO` gates — `I2S0_8CH` (`:944`), `I2S1_8CH` (`:2231`),
>   `I2S2_2CH` (`:897`) and `I2S3_2CH` (`:914`). That is exactly the
>   `CLK_IGNORE_UNUSED` fix Heiko called *"the correct fix"* below.
>
> So a `v7.2` board gets the MCLK gates **and** the fix that keeps
> `clk_disable_unused()` off them, and the analog-audio blast radius the last
> paragraph warns about no longer exists at this base. Keep the section: it is the
> record of a correct call at the time it was made, and it is the reason nobody
> needs to re-derive the prerequisite chain. **Do not re-open it as a candidate.**
...
2026-06-24 and, as of 2026-08-08, had **not landed**: `clk-rk3588.c` had exactly
one commit since 2026-04-19, `02b9b0bb6269` itself. Importing at that point meant
importing the known-buggy version with no follow-up available.

**That is no longer the situation, as of 2026-08-26.** The promised fix landed as
`946352b2f88fd` on 2026-06-28 — four days after it was promised, and three weeks
after the sweep that recorded it missing — and it is contained in the `v7.2` base.
The paragraph above is kept as written because it was true when it was written and
because it is why the `v7.1.7` import was refused; the box at the top of this
...
So the import would have traded an IOMMU-class bug we do not have for a silent loss
of headphone and mic audio on a board we ship. The re-open condition this paragraph
used to carry was "when the `CLK_IGNORE_UNUSED` follow-up is in mainline **and** the
base is still below the release that carries it" — and **that condition can no
longer be met, because its two halves are now mutually exclusive.** The follow-up is
in mainline (`946352b2f88fd`) and the base is `v7.2`, which contains it. The row is
therefore closed by absorption rather than re-opened: see the box at the top of this
section.
...
- Retire trigger: fired at v7.2 — retired, see registry
- Disposition: RETIRED
...
This sweep was run **differentially**: `docs/UPSTREAM-STATUS.md` was saved from
`HEAD` before any edit, every member row was assigned a route and a reachability
verdict up front, and a checker compared the result against that baseline — so a
row whose date moved without its source being reached is a build failure rather
than a reading error. All twenty-two rows were reachable; no status token changed.

| Source | How | Result |
|---|---|---|
| The pinned kernel tree at **`v7.2`** | Path, symbol and content checks per row, against the tag the pin actually resolves to | Counterparts still absent for `0003`, `0005`, `0006`, `0010`, `0011`, `0012`, `0026`, `0027`; no `rkvenc`/`vepu580` anywhere for the `0001` block; `system-uncached` still unregistered and `dma-heap.h` still has no removal API |
| Collabora `mainline-status.md` | **GitLab REST API**, plain `curl`, 2026-08-26 — **currently accessible without the Anubis gate**, and confirmed so by fetching the same URL twice, once with curl's default UA and once with `Mozilla/5.0`: both HTTP 200, both 37,103 bytes, zero challenge pages. **Browser fallback retained** for if the gate returns | `main` → commit `607a267502837967ed4a9a58bd9cdaed842a9509`. VEPU580 H.264 `WIP`, H.265 `TODO` — unchanged |
| The four tracked lore postings (`0005`, `0010`, `0011`, `0012`) and `0001`'s tracker | `https://lore.kernel.org/all/<message-id>/t.mbox.gz`, **plain `curl` only**. Sent with a `Mozilla/5.0` UA all five returned an Anubis challenge page under an HTTP **200** — a success code wrapping an interstitial, which is why a status-code-only check is not enough here | 19 / 3 / 2 / 5 / 15 unique messages. Newest traffic: `0005` 2026-08-11, `0010` 2026-05-10, `0011` 2026-06-03, `0012` 2026-08-08, `0001` 2026-07-24. **No merge, no reroll, no pickup on any of them** |
| `patchwork.kernel.org` API | `…/api/patches/?msgid=<msgid>` for the three `backports/` postings | `0010`: `changes-requested` (`linux-phy` 14494095), `new` (`linux-rockchip` 14494091). `0011`: `new` in both `linux-rockchip` 14585947 and `dri-devel` 14589721. `0012`: `new` (`dri-devel` 14582546). **`0005` returns zero results** — patchwork is still not exhaustive, exactly as the 2026-08-08 sweep found |
| Mainline containment | `api.github.com/repos/torvalds/linux/compare/<sha>...v7.2` per commit, plus a bounded per-path commit sweep of each member's file | **IN `v7.2`:** `d1162a5adbb5`, `54eff31301a0`, `5164f7e7ff8e`, `fb145be7964d`, `fd55edff8a0a`, `f7606400f19c`, `946352b2f88fd`, `0b31f297557f`, `be2b5b17b705`. **NOT in `v7.2`** (all `diverged`): `050febb6`, `433d8ad4`, `9c61998c9c85` — real commits, later merge window, and three plausible-looking citations this file now explicitly refuses |

**A note on that last row, because it is the sweep's most reusable lesson.** Three
of the twelve SHAs checked were commits that genuinely exist, genuinely touch the
right file, and are genuinely **not in this base**. A subject line and a date cannot
tell those apart from the ones that landed; only a containment check can. Resolve
every SHA against the pinned base before writing it into a row.

<details>
<summary>Sources checked for the previous sweep (2026-08-08, the `v7.1.7` base) — kept for the record</summary>
...
**The Collabora capture is a snapshot, not a live feed.** Re-capture it at every
base bump — the API route first, a real browser if the gate is back — and move the
"Last checked" dates in the same change. A stale date is the honest signal that a
row has not been re-verified, which is exactly why an unreachable row keeps its old
date instead of quietly taking the sweep's.

---

## Updating a row

1. Re-capture the Collabora table and re-resolve the row's Message-ID. The GitLab
   API route was ungated as of 2026-08-26 and plain `curl` is enough; keep a real
   browser as the fallback for when the Anubis gate returns. For lore, use
   `t.mbox.gz` with **curl's own User-Agent** — a browser-like UA draws the
   challenge page, served under an HTTP 200.
2. Update **Upstream status** and **Last checked** together. A status change with
   an unchanged date is not a check — and a date change with no source reached is
   worse, so if the source did not answer, leave **both** alone and record
   `re-check attempted <date> via <route>: unreachable` in the Notes instead.
